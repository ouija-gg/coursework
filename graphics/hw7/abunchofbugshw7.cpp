...
struct mline {
    point pt1, pt2;
    mline(point ipt1, point ipt2) {
        pt1 = ipt1;
        pt2 = ipt2;
    }
    mline addto(point inpt) {
        return line(this.pt2, inpt);
    }
    mline add2x(int nx) {
        return line(this.pt2, pt2.addx(nx));
    }
    mline add2y(int ny) {
        return line(this.pt2, pt2.addy(ny));
    void draw() {
...
struct gamewall {
    ...
    line outtl;
    line outl; */
    void addLinex(int nx) {
        lines.push_back(lines.end().add2x(ny));
    }
    void addLiney(int ny) {
        lines.push_back(lines.end().add2y(ny));
    }
    ...
    gamewall(int h, int w, int hs, int hw) {
        height = h;
        width = w;
        hole_size = hs;
        hole_width = hw;

        const int base = (scene.y - height)/2;
        const int top  = (base + height);
        const int left = (scene.x - width)/2;
        const int right= left+width;
        const point bot_bar = point(base, (scene.x - width)/2);
        point bot_bar_top = bot_hole.addy( (scene.y - hole_size)/2);

        point start = point(base, left );
        addLine( line(start, bot_bar));
        addLiney( bot_bar_top.y);
        addLinex( hole_width);
        addLiney( -hot_bar_top.y);
...
struct circle {
    ...
    void init() {
        ...
        static std::uniform_int_distribution<int>
            genx( wall.xr / 2 + 10 + 5 , wall.xr - r);
        // gen numbers between playable area in y direction
        static std::uniform_int_distribution<int> 
            geny( wall.yb + r, wall.yt- r);
...
void drawText( ) {
     ...
     glRasterPos2f( scene.xr/2, scene.yt/2);
     for (auto &k : msg)
...
int main(int argc, char** argv)
{
	...
	glutInitWindowSize(scene.xr,scene.yt); 			// intial pixel width and height of the display window
